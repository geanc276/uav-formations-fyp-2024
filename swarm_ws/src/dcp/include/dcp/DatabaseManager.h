...
public:
     ...
     * @param spec A VarSpec structure containing information about the producer ID, repetition count, and description.
     * @param length The length of the variable, corresponding to the size of the binary data stored in the 'Value' column.
     * @param value A vector of bytes (uint8_t) containing the binary data to be stored in the database.
     * @return The appropriate VARDIS_STATUS code
     */
    int create(VarSpec spec, VarLen length, std::vector<uint8_t> value);
     ...
     */
    bool updateDBEntryById(VarId varId, VarLen varLen, const std::vector<uint8_t>& value);
...
private:
    /**
     * @brief The real-time database (RTDB) that stores the VarDis variables
     */
    tbb::concurrent_unordered_map<VarId, DBEntry> dataBase;
    /**
     * @brief The next available VarId for a VarDis variable
     * Mainly used to manually create VarSpec objects
     */
    VarId nextVarId = 1;
    /**
