...
#define VARDISPAR_MAX_PAYLOAD_SIZE 1000
#define VARDISPAR_BUFFER_CHECK_PERIOD 1000ms
...
struct TimeStamp {
    ...
    TimeStamp() : time(std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::system_clock::now().time_since_epoch())) {}

    // Conversion operator from integer to TimeStamp
    TimeStamp(std::int64_t nanoseconds) : time(nanoseconds) {}
};
...
struct VarSpec {
    ...
    NodeIdentifier prodId;     // Producer identifier associated with the variable
...
struct VarUpd {
    ...
    std::vector<uint8_t> Value;   // Value of the update as a vector of bytes
    ...
    VarUpd(VarId id, VarSeqNo seqNo, VarLen length, std::vector<uint8_t> value)
...
inline std::ostream& operator<<(std::ostream& os, const DBEntry& entry) {
    ...
    os << "VarSeqNo: " << entry.seqno << ", "
       ...
       << "ExistsInDB: " << entry.existsInDB;
    return os;
}

// Overload the stream insertion operator for the VarUpd Update struct, allows for printing
inline std::ostream& operator<<(std::ostream& os, const InformationElement& informationElement) {
    os << "ieType: " << informationElement.ieHeader.ieType << ", "
       << "ieNumRecords: " << informationElement.ieHeader.ieNumRecords << ", "
       << "ieTimestamp: " << informationElement.ieHeader.ieTimestamp << ", "
       << "ieSeqNum: " << informationElement.ieHeader.ieSeqNum << ", "
       << "ieList: [";
